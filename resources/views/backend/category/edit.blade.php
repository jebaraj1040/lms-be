...
<div class="seperator-header layout-top-spacing ">
    <h3 class="">Edit Course Category</h3>
</div>
...
<div class="row layout-top-spacing" id="cancel-row">
    <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
        <div class="widget-content widget-content-area br-6 p-4">
            <div class="col-lg-11">
            <form style="min-height:500px;" id="category_edit_form" method="POST" action="{{route('admin.categories.update',$category->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group row mb-4">
                    <label for="name" class="col-xl-3 col-sm-3 col-sm-2 col-form-label">Name <span style="color:red;">*</span></label>
                    <div class="col-xl-8 col-lg-9 col-sm-10">
                        <input type="text" class="form-control alphabets" id="name" maxlength="50" name="name" placeholder="Enter name" value="{{old('name',$category->name)}}">
                    </div>
                </div>
                <div class="form-group row mb-4">
                    ...
                    <div class="col-xl-8 col-lg-9 col-sm-10">
                        <select id="department_id" name="department_id" class="form-control">
                            <option value="" {{(old('department_id',$category->department_id)=='')?'selected':''}}>Select Department</option>
                            @if ($departments)
...
<script type="text/javascript">
        ...
        @if($category->id!="")
            @php
                $category_image_path = !empty($category->image)?asset('/category_image/') . '/' . $category->image:'';
            @endphp
        @endif
        var presetFiles = @if(!empty($category_image_path)) ["{{ $category_image_path }}"] @else [] @endif;
...
$(document).ready(function () {
    $(document).on('click', '.submit_category_form', function (e) {
        ...
        if ($("form#category_edit_form").valid()) {
